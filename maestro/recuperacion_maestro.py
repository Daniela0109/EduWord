...
class Recuperacion_Maestro:
    def __init__(self, recuperacion, ventana_loguin):
        self.recuperacion = recuperacion
        self.recuperacion.title("Recuperacion de Contraseña Maestro")
        ...
        label_presentacion = tk.Label(self.recuperacion, text="Recuperación de Contraseña", font=("Comic Sans MS", 16, "bold")).pack(pady=20)

        label_correo = tk.Label(self.recuperacion, text="Correo del maestro:", font=("Comic Sans MS", 12)).pack()
    ...
    def iniciar_recuperacion(self):
        ...
        if not correo:
            messagebox.showwarning("Campo vacío", "Por favor, ingresa el correo del maestro.")
            return

        try:
            query = db.collection('maestro').where('correo', '==', correo).limit(1)
            results = query.get()

            if results:
                maestro = results[0].to_dict()
                contraseña_cifrada = maestro.get('password')
                ...
                if contraseña_cifrada:
                    ...
                    messagebox.showinfo("Correo enviado", "La contraseña ha sido enviada exitosamente al correo del maestro.")
                else:
                    messagebox.showwarning("Sin contraseña", "El usuario no tiene una contraseña registrada.")
            else:
                messagebox.showerror("Correo no encontrado", "No se encontró un maestro con ese correo.")
